...
data class LikedTrack(
    ...
    @ColumnInfo(name = "spotify_url") val spotifyUrl: String? = null,
...
interface LikedTrackDao {
    ...
    @Query("SELECT * FROM liked_tracks WHERE id = :id")
    suspend fun getById(id: Long): LikedTrack?

    /** Fills only missing links so user edits are never overwritten. */
    @Query("""
        UPDATE liked_tracks SET
            spotify_url = COALESCE(spotify_url, :spotifyUrl),
            apple_music_url = COALESCE(apple_music_url, :appleMusicUrl),
            spotify_id = COALESCE(spotify_id, :spotifyId),
            updated_at = :updatedAt
        WHERE id = :id
    """)
    suspend fun fillLinks(
        id: Long,
        spotifyUrl: String?,
    ...
    @Query("""
        UPDATE liked_tracks SET
            ...
            spotify_id = COALESCE(:spotifyId, spotify_id),
            bpm = :bpm,
            energy = :energy,
            valence = :valence,
    ...
    @Database(entities = [LikedTrack::class], version = 6, exportSchema = false)
    ...
    companion object {
        ...
        private val MIGRATION_5_6 = object : Migration(5, 6) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                ).forEach { db.execSQL("ALTER TABLE liked_tracks ADD COLUMN $it") }
            }
        }
